...
class ColoredShapeNNet(nn.Module):

    def __init__(self, w1=3, w2=5):
        super(ColoredShapeNNet, self).__init__()
        
        # Input features = 2, output features = 2
        self.hidden_layer = nn.Linear(2, 2, bias=True)
        self.hidden_layer.weight = nn.Parameter(torch.tensor([[1.0, -1.0],[1.0, 1.0]]))
        self.hidden_layer.bias = nn.Parameter(torch.tensor([-3.0, -3.0]))
        
        # Input features = 2, output features = 1
        self.output_layer = nn.Linear(2, 1, bias=False)
        self.output_layer.weight =  nn.Parameter(torch.tensor([[float(w1), float(w2)]]))

    def forward(self, x):
        # Apply activation functions in the hidden layer to the inputs:
        x = self.hidden_layer(x)
        # Apply activation function in the output layer to the hidden layer
        # outputs:
...
net = ColoredShapeNNet(w1=w1, w2=w2)
...
for s in s_range:
    for c in c_range:
        ...
        inputs = torch.tensor([float(s), float(c)])
